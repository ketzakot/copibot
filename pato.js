...
async function starts() {
	...
	console.log('>', '[',color('INFO','blue'),']','Gracias por usar killer bot...')
	turbo.on('qr', () => {
	...
	turbo.on('connecting', () => {
	console.log(color('> [ INFO ]', 'white'), color('esperate estoy agarrando señal...'))
	})
	turbo.on('open', () => {
	console.log(color('> [ INFO ]', 'white'), color('Listo papu'))
	})
...
//Welkom
			...
			teks = `🤡No se quien raios ⚡eras pero mejor que te fuistes😹\n`
			teks += `●     × •-•-•-•⟮ ◆ ⟯•-•-•-• ×       ●\n`
...
　        teks += ` .　ﾟ • ﾟ   •　。ﾟ    • ඞ   。　ﾟ .\n`
			...
			teks = `*wow wow un nuevo admin bienvenid@*
...
\`\`\`Grupo :\`\`\` ${mdata.subject}`
			...
			let buff = await getBuffer(ppimg)
			teks = `*Si vez lo que hisiste ahora ya no eres akmin*
...
turbo.on('chat-update', async (mek) => {
	try {
		...
		const ownerNumber = ['573508419687@s.whatsapp.net'] //turbo owner
//SETTING WeA
...
//
		...
		mess = {
			wait: 'Espera dejame que trabajo en eso va?',
			success: 'Comando con  éxito',
			wrongFormat: 'puedes escribir bien el formato por favor!!!🙄?',
			error: {
				stick: 'lo siento te falle no pude convertir en sticker ',
				Iv: 'pon bien el pinche link '
			},
			only: {
				group: '*Lo siento este comando solo puede ser usado en un grupo*',
				admin: '_Solo los admin facheritos pueden usar este comando_',
				Badmin: 'Ptmd sin admin no puedo hacer nada :v rolen admin'
			}
		}
		const isUrl = (url) => {
...
//FUNCTION
            ...
            turbo.sendMessage(mek.key.remoteJid,`El bot esta modo privado activado, comunicate con el creador ketza ntr\n\n❮ KillerBot Ketzakot Oficial❯`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*OFFLINE*", 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}})
            }
            }   
        if (mek.key.remoteJid.endsWith('@g.us') && offline) {
        if (!mek.key.fromMe){
        ...
        addafk(mek.key.remoteJid)
        heheh = ms(Date.now() - waktu)
        turbo.sendMessage(mek.key.remoteJid,`El bot esta modo privado activado, comunicate con el creador Ketzakot ntr\n\n❮ crazy killer Bot by ketzakot❯`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*OFFLINE*", 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}})
          }
        }
            }
          }
...
⚠︎➪ ${prefix}apagar
 ❖ ── ✦ ──『✙』── ✦ ── ❖
『𝗜𝗻𝗳𝗼 𝗱𝗲𝗹 𝗕𝗼𝘁』
⚠︎➪ Hits de hoy : ${hit_today.length}*
⚠︎➪ Celular : ${device_manufacturer}
⚠︎➪ Modelo : ${device_model}
�̜͉̰͖̙̙͈͕̰̞̤͇̘̽̏̈ͪ̑ͤͧ̆̄̅̚͘͢͝҉̷̵̸͞҉-̵̵̡̧̛̠̲͇̱̝̺̺̦̥͑̈ͥ̓̌̉͒͋ͭ̀͋̒ͯͬͅ-̸̶̵̡̢̨̛̀̕͜͜͡͡͞-̶̲̦͈̳͖͔̣̤̥͕̙̳̠̠̘ͦ̈͡҉̷̷̴̶̴̨̧̛́͟͢͝҉̷̵̢͡-̴̸̢̛̛̗̝̱̙̯͓͙͍͚͈̰͔̼̝͙͈̫͒̄̒̓̓ͭ҉҉̷̴̀͘-̢̢̧̞̯̤̥̬̮̻̃͊͊̓͋ͮ́̌ͯͭ̽͂ͩͥͣ̀ͭ͗ͯ̈͡-̵̷̴̶̸̨̢̢̞̪̙͇̩̮̖̯̳̞͆̍ͦ̌̆̽ͯ̍̆ͤ͢͡͠͠͡͠ͅ͏̷̸͏҉-̸̘͚͍͈̫̠̱͚̎̐̒̇ͥ͑̅̅̿̈́̉͒̂͏̵̴̸̷̡̨̛̀̀͘͘͟͢͜͡͞͡-̧̭͍̙̩̞ͧ̎͂̍ͩ̈͒̌͆҉̨͘͏̸
...
case prefix+ 'avatar':
		...
		case prefix+ 'waifu':
			...
			turbo.sendMessage(from, nyed, image, { caption: '✪ BY CRAZY KETZAKOT ✪', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
...
break
	...
	case prefix+ 'husbu':
			...
			turbo.sendMessage(from, nye, image, { caption: '❮ ❮ BY CRAZY KETZAKOT ❯❯', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
...
case prefix+ 'pinterest':
    ...
    case prefix+ 'anime':
            ...
            turbo.sendMessage(from,media,image,{quoted:mek,caption:'✪ BY CRAZY KETZAKOT ✪'})
            }
            )
            .catch((error) => {
            console.log(error); 
...
case prefix+ 'getbio':
	   ...
	   case prefix+ 'getpic':
		...
		turbo.sendMessage(from, thumb, MessageType.image, {caption: '✪ BY CRAZY KETZAKOT ✪'})
	        }
		break
case prefix+ 'setbio':
	        if (!isMe) return reply('Esté comando solo puede usado por mi')
...
case prefix+  'stats':
                ...
                await wa.sendFakeStatus(from, texxt, "✪ BY CRAZY KETZAKOT ✪")
                break
case prefix+ 'block':
...
quoted: mek, caption: `✪ BY CRAZY KETZAKOT ✪`
})
} catch {
reply(ptbr.erro())
}
...
//CONVERTER
    case prefix+ 'toimg':
			...
			fakethumb(buffer,'✪BY CRAY KETZAKOT✪')
			fs.unlinkSync(ran)
			})
			break
    case prefix+ 'tovid':
    case prefix+ 'tovideo':
            ...
            sendMediaURL(from,res.result,'✪ BY CRAZY KETZAKOT ✪')
            })
            }else {
...
case prefix+ 'creador':
			case prefix+ 'dueño':
                ...
                break
 case prefix+ 'igtitans':
        her = `INSIGNIAS OBLIGATORIAS

[HOMBRES]

🐉⃢⃚⃜͜͡➫Nombre˿⃖🔥⃟⃚᪵⃜༄᪵
